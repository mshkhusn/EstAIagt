# app.py （AI見積もりくん２ / Neon Splash Theme：components.v1.html 版・完全コピペOK）
# - GPT系のみ（OpenAI）
# - チャットで要件収集 → ボタンで見積り生成（不足は推測）
# - 再生成は「過去のチャット＋新要件」を反映
# - 追加入力時は一旦プレビューを消去
# - 生成後はチャット入力欄の直上にヒント表示
# - CSSは components.v1.html で注入（テキスト表示問題を回避）
...
# =========================
# CSS（components.v1.html で強制注入：ここが“出力されない”肝）
# =========================
components.v1.html("""
...
html, body { background: var(--bg); color: var(--text); font-family: "Noto Sans JP","Zen Maru Gothic","Bungee",system-ui,-apple-system,Segoe UI,Roboto,sans-serif !important; }
...
"""
    ...
    if st.session_state["df"] is not None:
        ...
        st.caption("テンプレ内の明細開始セルに `{{ITEMS_START}}` を置いてください。")
