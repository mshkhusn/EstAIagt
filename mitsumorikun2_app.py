# app.py（AI見積もりくん２ / Neon Splash Theme・完全版）
# - OpenAI GPT系のみ（gpt-4.1を使用）
# - チャットで要件収集 → ボタンで見積もり生成（不足は推測）
# - 追加入力時は一旦プレビューを消去 → 再生成で「履歴＋新要件」を反映
# - 見積もり生成後、入力欄直前に「続けて精度アップ」のヒントを表示
# - Excelダウンロード／DD見積テンプレ出力に対応
# - CSSは st.markdown(..., unsafe_allow_html=True) で**グローバル注入**
#   （ここが効かないときは、バッククォート混入や unsafe_allow_html の付け忘れが原因です）
...
# =========================
# ネオン×インク（Splatoonライク）CSS を**グローバル注入**
# ※ ここがテキスト表示される時は「unsafe_allow_html=True」が抜けているか、
#    文字列に ```（バッククォート）が混入している可能性があります。
# =========================
st.markdown("""
...
if st.session_state["chat_history"] is None:
    st.session_state["chat_history"] = [
        {"role": "system", "content": "あなたは広告クリエイティブ制作のプロフェッショナルです。相場感をもとに、必要なヒアリングを行い、概算見積もりを作成します。"},
