# app.py（Gemini 2.5 Flash 専用・安定版）
# ・OpenAI/他モデル/response_schema 等の記述は全削除
# ・2.5 Flash を generate_content(str) で呼び、堅牢パーサで JSON 抽出
# ・既存の見積UI/計算/Excel/テンプレ出力は維持
...
STRICT_JSON_HEADER = (
    ...
    "説明や前置きは禁止です。生成が難しい場合も {\"items\": []} を返してください。"
)
...
def llm_generate_items_json(prompt: str) -> str:
    try:
        model = _gemini25_model_simple()
        resp = model.generate_content(prompt)  # ← 最小経路（strで送る）
        try:
            st.session_state["gemini_raw_dict"] = resp.to_dict()
        except Exception:
            st.session_state["gemini_raw_dict"] = {"_note": "to_dict() failed"}
        raw = (getattr(resp, "text", None) or "").strip()
        if not raw:  # 念のため chat 経路で一回だけ再試行
